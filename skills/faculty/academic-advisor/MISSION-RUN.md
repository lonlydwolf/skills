...
The advisor's first invocation, once per course — and the one exemption from the orient step's reading: there is no `roadmap.md` yet, so the grilling itself is the orient and there is no header to set.
...
Every subject — and every territory question that surfaces under one — leaves the run in exactly one of two states: **settled** by the user in the conversation, or **ticketed** open in `.roadmap/issues/` ([PLANNING-TICKET-FORMAT.md](PLANNING-TICKET-FORMAT.md)). A question the conversation didn't settle is ticketed, not answered on the user's behalf. Fire each research ticket as a `/research` subagent during the run, findings landing in `.roadmap/research/`; open tickets outliving the run are normal — they are the first checkpoint's business. The mission record lives in ticket 01 (see the format doc); the distillate goes to `roadmap.md`.
