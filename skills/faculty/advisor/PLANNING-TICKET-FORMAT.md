...
- **Two types only.** `research` (a fact outside the course folder, resolvable by a `/research` subagent) or `grilling` (a decision to work through with the user). Planning has no prototype or task tickets — if manual work is blocking the plan, that's a Notes line in `roadmap.md`, not a ticket type.
